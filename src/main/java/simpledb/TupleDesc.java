...
public class TupleDesc implements Serializable {
    ...
    public TupleDesc(Type[] typeAr, String[] fieldAr) {
        if (typeAr == null || typeAr.length < 1) {
            throw new IllegalArgumentException("typeAr should contain at least one entry");
        }
        this.items = new ArrayList<>();
        for (int i = 0; i < typeAr.length; i++) {

            // If there are fewer fieldnames, pad with nulls
            String fieldName = i < fieldAr.length ? fieldAr[i] : null;
            this.items.add(new TDItem(typeAr[i], fieldName));
        }
    }

    /**
     * Helper constructor for directly assigning a TDItem lists,
     * since TDItem is private class, this constructor is private too
     * @param items
     */
    ...
    public int fieldNameToIndex(String name) throws NoSuchElementException {
        ...
        for (; idx < this.items.size(); idx++) {
            if (name.equals(this.items.get(idx).fieldName)) {
                break;
            }
        }
    ...
    public int getSize() {
        ...
        for (int i = 0; i < this.items.size(); i++) {
            sizeInBytes += this.items.get(i).fieldType.getLen();
        }
    ...
    public static TupleDesc merge(TupleDesc td1, TupleDesc td2) {
        ...
        newItems.addAll(td1.items);
        newItems.addAll(td2.items);

        TupleDesc newDesc = new TupleDesc(newItems);
        newDesc.items = newItems;
